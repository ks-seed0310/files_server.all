class xorEncrypt{
    encrypt2(value,key=undefined,k2=undefined){
...
            return this.encrypt2(value.toString())
...
            let xk=(Math.imul(((prev^kp)>>>0)+998244353,0x85ebca6b)>>>0)&4294967295
            let val=u32[i]^xk
...
            if (b.byteLength%4!==0){
                val_bt=b[0]
                const bs=b.subarray(1).slice()
                u32=new Uint32Array(bs.buffer)
            }else{
...
            let val=u32[i];
            val=(val>>>5)|(val<<(32-5))
            let xk=Math.imul(((prev^kp)>>>0)+998244353,0x85ebca6b)>>>0
            u32[i]=(val^xk)>>>0
        }
        let u32b=new Uint8Array(u32.buffer.slice(0),0,u32.length*4)
